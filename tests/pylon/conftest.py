"""Shared collection rules for the Pylon capability tests."""
...
# `mcp` and `fastmcp` are gated on the `pylon` extra, so slim CI runs (no extras) can't import
...
collect_ignore = ['test_pylon.py'] if any(importlib.util.find_spec(name) is None for name in _REQUIRED) else []
