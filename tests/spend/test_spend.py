...
def _hook_ctx(guard: SpendLimits[Any]) -> RunContext[Any]:
    """A `wrap_model_request` ctx bound the way core binds the hook ctx, so `ctx.emit` is allowed."""
    run_ctx = _run_ctx()
    run_ctx._capability = guard
    run_ctx.capabilities = {'spend_limits': guard}
    return run_ctx
...
class TestOrdering:
    ...
    async def test_every_committed_provider_response_is_counted_instead_of_the_middleware_result(self):
        ...
        result = await guard.wrap_model_request(_hook_ctx(guard), request_context=request_context, handler=handler)
    ...
    async def test_identical_provider_responses_accrue_separately_and_replay_once(self):
        ...
        for _ in range(2):
            ...
            await guard.wrap_model_request(_hook_ctx(guard), request_context=request_context, handler=handler)
