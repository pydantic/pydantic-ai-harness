...
collect_ignore = [] if _HAS_DAYTONA else ['test_backend.py', 'test_daytona_workspace.py']
...
class _PoisonedDaytona(types.ModuleType):
    ...
    def __getattr__(self, name: str) -> object:  # pragma: no cover - tripwire, hit only by a misbehaving test
        raise AssertionError(
            'A daytona_workspace unit test touched the real `daytona` package. Use the `fake_daytona` fixture.'
        )
...
def _no_real_daytona(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.setattr('pydantic_ai_harness.daytona_workspace._backend.daytona', _PoisonedDaytona('daytona'))
    yield
...
if _HAS_DAYTONA:  # pragma: no branch - the fixture requires the SDK-backed fake
    ...
    def fake_daytona(monkeypatch: pytest.MonkeyPatch) -> FakeDaytona:
        fake = FakeDaytona()
        monkeypatch.setattr('pydantic_ai_harness.daytona_workspace._backend.daytona', daytona)
