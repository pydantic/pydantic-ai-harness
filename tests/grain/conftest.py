"""Shared collection rules for the Grain capability tests."""
...
# `mcp` and `fastmcp` are gated on the `grain` extra, so slim CI runs (no extras) can't import
...
collect_ignore = ['test_grain.py'] if any(importlib.util.find_spec(name) is None for name in _REQUIRED) else []
