...
def test_daytona_live_runs_the_live_tier_with_a_step_scoped_secret() -> None:
    ...
    job_block = lines[lines.index('  daytona-live:') : run_index]
    assert '      - run: uv sync --locked --group dev --extra daytona' in job_block
    # A missing secret fails the job instead of skipping every live test.
    assert "      DAYTONA_REQUIRE_LIVE: '1'" in job_block
