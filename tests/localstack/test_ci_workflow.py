...
def test_sprites_live_runs_the_live_tier_with_a_step_scoped_secret() -> None:
    ...
    job_block = lines[lines.index('  sprites-live:') : run_index]
    assert '      - run: uv sync --locked --group dev --extra sprites' in job_block
    assert "      SPRITES_REQUIRE_LIVE: '1'" in job_block
