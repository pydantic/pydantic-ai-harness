...
| Tool | Purpose |
|---|---|
| `read_file` | Read a UTF-8 text file with line numbers and a content hash. NUL-containing files are detected and not dumped; other undecodable bytes use replacement characters. Supports `offset`/`limit` paging. A truncated window ends with `[truncated: ...]`; lines longer than 2000 characters are clipped in the returned string. |
