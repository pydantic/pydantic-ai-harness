...
Compose search with Web Search Agents when you need both:
...
Factory-built `AsyncNimble` clients are closed after each agent run via the
capability `after_run` hook.
...
Expect **3–15 minutes** for deeper tiers. Observed rough timings (not SLAs):
`low` can finish in tens of seconds (sometimes with weak sources / low
confidence); `medium` is often ~90–160s. Never block-poll inside one tool call —
...
`GET /v2/agents/{id}/runs/{run_id}/events` (SSE). `agent_run_start` accepts
`enable_events` for forward compatibility, but this capability does **not**
expose an events subscription tool — use start/status/result.
