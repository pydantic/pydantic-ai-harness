...
`used_tokens` counts message parts, the same way the triggers do. Instructions (`ModelRequest.instructions`) and tool schemas are outside that count, so the reading is lower than what the provider bills; tool-schema accounting is tracked in [#100](https://github.com/pydantic/pydantic-ai-harness/issues/100).
