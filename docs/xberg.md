---
title: Xberg
description: Let a Pydantic AI agent read PDFs, Office files, images, audio, and archives through a Xberg server.
---
...
[tool approval](/ai/tools-toolsets/toolsets/#requiring-tool-approval) when extraction should
...
[`Agent(toolsets=[...])`](/ai/tools-toolsets/toolsets/) and other toolset wrappers. The
...
`max_output_bytes` past a [Tool Output Limits](tool-output-limits.md) band: an oversized return then spills
to a store the model reads with `read_tool_result`, rather than being cut.

Xberg also ships an MCP server (`xberg mcp`). Pydantic AI's own [`MCP`](/ai/capabilities/mcp/)
...
[`PrefixTools`](/ai/capabilities/overview/):
...
::: pydantic_ai_harness.xberg.Xberg

::: pydantic_ai_harness.xberg.XbergToolset
