...
description: Let TypeSafe's Jev pick each run's model, thinking effort, and capabilities from an allowlist.
---
...
`models` takes the same entries as the [sub-agents](subagents.md) model menu: a model ID, a `Model`, or a `ModelOption`. The picked entry becomes the run's model, unless the run was given one with `Agent.run(model=...)`, which takes precedence. The picked effort goes on the run as `ModelSettings(thinking=...)`, and a `ModelOption.settings` overrides it, so an entry that must always think hard can say so. A model whose profile does not support thinking ignores the setting.
