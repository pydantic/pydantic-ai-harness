...
description: Let a picker model, TypeSafe's Jev by default, compose a sub-agent for each prompt from a model menu and a capability allowlist.
---
...
| `skills` | [Skills](skills.md) | `SKILLS_DIRECTORY` (`.agents/skills`) | that directory exists and the `skills` extra is installed |
...
Approval inside the sub-agent has to happen while it runs, with an async `ToolGuardrail` guard in `shared_capabilities` that asks the user (see [human in the loop](guardrails.md#human-in-the-loop)). Deferred approval (`GuardrailResult.approve()`, `requires_approval=True`) ends a run with `DeferredToolRequests`, which the sub-agent's text output cannot carry back.
