...
[You.com APIs](https://docs.you.com/). Use it when an agent needs current web
results, full known pages, or cited synthesis across several sources.
...
unified results from web and news sources. Each result carries `kind='web'` or
`kind='news'` so callers can recover the provider section. A response without a
...
questions that span multiple domains. The returned `warnings` list reports
source-access issues or partial results.
