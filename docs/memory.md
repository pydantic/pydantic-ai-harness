...
The two write disjoint tables, so they coexist in one file. Swap both connections for a pool and the same pairing runs on PostgreSQL; see [Step Persistence](step-persistence.md) for what the message side records.
