...
With [Modal](modal-sandbox.md), for example (the `python:3.12` image includes `git`):
...
- **Skills** are read from the workspace at run start and loaded with a `load_skill` tool. [`Skills(workspace=LocalWorkspaceBackend('/app'))`](skills.md) reads them from somewhere else.
