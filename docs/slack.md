...
Give each `Slack` its own `id`, and wrap each in [`PrefixTools`](/ai/capabilities/prefix-tools/) so the two copies of Slack's tool catalog get different names. The prefix is also how the model tells the workspaces apart.
