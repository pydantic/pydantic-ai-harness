---
title: Harness CLI
description: A terminal client for the Coder harness. Runs one prompt against Coder in the current directory and renders the run as it happens.
---

# Harness CLI

A terminal client for the [Coder](coder.md) harness. `harness -p "..."` runs one prompt against `Coder` in the current directory and renders the run as it happens: streamed Markdown for the model's text, one line per tool call and result.
...
The CLI adds no spans of its own, and neither does `CliBridge`: rendering records no decision that core's spans do not already show. Core's [instrumentation](/ai/capabilities/instrumentation/) covers the run: `logfire.instrument_pydantic_ai()` before `main()` traces every model and tool call.
