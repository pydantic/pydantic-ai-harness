---
title: Harness CLI
description: A terminal client for the Coder harness. Runs one prompt against Coder in the current directory and prints the response.
---

# Harness CLI

A terminal client for the [Coder](coder.md) harness. `harness -p "..."` runs one prompt against `Coder` in the current directory and prints the response.
...
The CLI adds no spans of its own. Core's [instrumentation](/ai/capabilities/instrumentation/) already covers the run: `logfire.instrument_pydantic_ai()` before `main()` traces every model and tool call.
