...
def usage_table(usage: SessionUsage) -> Table:
    ...
    table = Table(header_style=theme.current().accent, border_style=theme.current().muted)
    ...
    table.add_section()
    table.add_row('Total', *_cells(usage.total, cached=cached), style=theme.current().accent)
    return table
