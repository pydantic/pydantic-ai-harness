    ...
    async def login(self) -> SecretStr:
        ...
        try:
            async with server, asyncio.timeout(self.timeout):
                self.console.print(
                    'Sign in to OpenRouter in your browser. Waiting up to five minutes.', style=theme.current().info
                )
                ...
                try:
                    ...
                    opened = False
                if not opened:
                    self.console.print('Open the URL above manually.', style=theme.current().warning)
