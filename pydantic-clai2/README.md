...
The terminal footer starts with the current working directory, using `~` for
your home directory. It refreshes as the shell's working directory changes and
shows `?` if that directory is unavailable. The footer also shows the selected
model, activity, latest reported context tokens, and streamed output estimate, including text, thinking, and
