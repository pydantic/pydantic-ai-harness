...
A command `timeout` counts from the call, so it includes acquiring the sandbox. Sandbox creation itself is not interrupted, since E2B may already have made the sandbox; a deadline that passes during creation raises once creation returns, with the sandbox recorded in `ref`. On timeout, cancellation, or a failure reading the result, the backend kills the command's process. A process the command started in the background outlives that kill until the sandbox ends. Commands buffer their complete output, and a timeout error carries the output collected so far.
