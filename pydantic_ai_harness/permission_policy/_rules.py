...
Two matcher philosophies combine here, following the survey referenced in this capability's
README:
...
- **opencode's last-match-wins ordering** for user rules: later rules override earlier ones,
  ...
  is allowed). No action-precedence lattice to reason about.
- **Codex's most-restrictive-wins** to merge the user-rule verdict with the built-in
  command-safety verdict: `deny > ask > allow`. This is what makes the flag denylist in
  `_command.py` "override" a broad user allow -- a user `allow git *` cannot green-light
  `git -C /etc ...`.
"""
...
class Rule:
    ...
    - `command`: a whitespace-separated *prefix* matched against a shell tool's command with
      ...
      (`git status` matches `git status -sb` but not `git status-evil` or `git push`). Only
      meaningful for shell-class tools; a non-shell call never matches a `command` rule.
...
class Decision:
    ...
    reason: str
    retryable: bool
    source: str  # 'rule', 'command-safety', 'context-policy', or 'default'
...
def more_restrictive(current: Decision, other: Decision) -> Decision:
    ...
    Exposed so a third channel (e.g. `PermissionPolicy.context_policy`) can tighten the
    rule/command-safety decision without ever loosening it.
    """
...
__all__ = ['Decision', 'Rule', 'bare_name_verdict', 'last_matching_rule', 'more_restrictive', 'resolve']
