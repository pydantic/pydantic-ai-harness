...
from ._command import PreparedCommand, Verdict, prepare_command
from ._rules import Decision, Rule, bare_name_verdict, more_restrictive, resolve
...
# marker load-bearing for the app.
...
# A `context_policy` sees the run context, tool name, and validated args and returns a `Verdict`
# to contribute a decision from the caller's identity/tenant/role, or `None` to abstain. Sync or async.
ContextPolicy = Callable[[RunContext[AgentDepsT], str, dict[str, Any]], 'Verdict | None | Awaitable[Verdict | None]']
...
class PermissionPolicy(AbstractCapability[AgentDepsT]):
    ...
    - **allow** -- the tool runs.
    - **deny** -- the call is blocked and an explanatory message is returned to the model (it
      is told whether re-requesting with justification can help, or the action is never
      allowed). With `deny_removes_tool=True`, a tool denied *regardless of arguments* is also
      removed from the model's toolset entirely (the Claude Code bare-name-deny pattern).
    ...
    context_policy: ContextPolicy[AgentDepsT] | None = None
    """Optional decision channel keyed on the run context -- caller identity, tenant, or role in
    `ctx.deps`, or the delegation chain -- for access decisions the tool name and args alone cannot
    express. Receives the `RunContext`, tool name, and validated args, and returns a verdict
    (`'allow'`/`'ask'`/`'deny'`) to contribute, or `None` to abstain. Its verdict is merged with the
    rule and command-safety channels **most-restrictive-wins**, so it can tighten a decision (deny a
    call a rule would allow) but never loosen one. Sync or async."""

    _pending_asks: dict[str, Decision] = field(default_factory=dict[str, Decision], repr=False, compare=False)
    """Tool-call IDs this instance itself raised `ApprovalRequired` for, keyed to the decision
    that triggered it. `ctx.tool_call_approved` is scoped to the *tool call*, not to which
    capability's `ask` it resolves -- a call gated by an unrelated mechanism (a core
    `requires_approval=True` tool, or a different capability's own deferred approval) can
    report `tool_call_approved=True` on a retry without this policy ever having resolved
    anything for it. Trusting that flag unconditionally would let a foreign approval silently
    satisfy this policy's own (possibly still-outstanding) `ask`. We only treat the call as
    approved *for us* when we recognize its `tool_call_id` as one we ourselves put on hold;
    otherwise we re-decide and raise our own `ApprovalRequired`, scoped to this policy."""
    ...
    async def prepare_tools(self, ctx: RunContext[AgentDepsT], tool_defs: list[ToolDefinition]) -> list[ToolDefinition]:
        ...
        for tool_def in tool_defs:
            ...
            if self.add_escalation_note and self._is_guarded(tool_def.name):
                description = ((tool_def.description or '').rstrip() + _ESCALATION_NOTE).lstrip()
    ...
    async def wrap_tool_execute(
        ...
        *,
        ...
        if context_policy is not None:
            verdict = context_policy(ctx, tool_def.name, args)
            if inspect.isawaitable(verdict):
                verdict = await verdict
            if verdict is not None:
                decision = more_restrictive(
                    decision,
                    Decision(verdict, reason='matched context policy', retryable=True, source='context-policy'),
                )
        if decision.verdict == 'allow':
            return await handler(args)
        if decision.verdict == 'deny':
            return self._deny_return(tool_def.name, decision)
        # ask -- only trust an approval this policy itself put on hold; see `_pending_asks`.
        if ctx.tool_call_approved and ctx.tool_call_id is not None and self._pending_asks.pop(ctx.tool_call_id, None):
            return await handler(args)
        if ctx.tool_call_id is not None:
            self._pending_asks[ctx.tool_call_id] = decision
