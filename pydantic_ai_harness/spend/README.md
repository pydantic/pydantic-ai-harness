...
Migration: `on_spend` remains supported but is deprecated. It still fires after each response `SpendLimits` records, sync or async, with a `SpendSnapshot` -- including one that `on_unpriced='raise'` is about to reject, since a report that skipped exactly the unpriced responses would be missing the ones worth knowing about -- and carries the response's `usage` unchanged, so cache reads and writes are available without this capability modelling them. Under durable execution, orchestration can replay it even though the journaled accrual ran only once. Move its callback body to a `SpendRecordedEvent` subscription, keeping the replacement idempotent for the same reason.
