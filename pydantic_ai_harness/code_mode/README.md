...
to quote a shell command inside a generated Python string. Their non-command tools (`read_file`,
`check_command`, and so on) are folded into `run_code` like any other tool. You can control which
eligible tools go through the sandbox:
