...
`--model` takes any name Pydantic AI's [`infer_model`](https://pydantic.dev/docs/ai/models/overview/) accepts. It overrides the config file's `model`, which defaults to `anthropic:claude-fable-5`. The provider's API key comes from the environment as usual; a missing key exits with the provider's own message.
...
- A delegation renders from the `sub_agents.*` events `SubAgents` emits: `>> worker (fast) task...` as the child run starts (the menu key in parentheses when one applies), and `<< worker (2.1s) output...` when it ends, replacing the generic result line. A delegation that did not finish cleanly names its outcome first (`<< worker timeout (30.0s) ...`, `budget`, `failed`, `contained`) and shows the message the parent model was given, so the retry prompt it may raise is not printed a second time. Nothing the child does is rendered in between until child streaming lands.
