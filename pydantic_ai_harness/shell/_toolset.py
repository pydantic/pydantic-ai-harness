    ...
    async def _cwd_for(self, ctx: RunContext[AgentDepsT]) -> str:
        ...
        if self._persist_cwd and ctx.run_id is not None:
            ...
            try:
                ...
                if posixpath.isabs(recorded) and (await ctx.workspace.stat(recorded)).is_dir:
                    return recorded
            except (FileNotFoundError, OSError, UnicodeDecodeError):
                pass
        return self._cwd if ctx.run_id is None and self._cwd is not None else await ctx.workspace.working_dir()
