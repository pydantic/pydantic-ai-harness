...
1. **Local checkout** -- when `local_docs_path` (or the `PYDANTIC_AI_HARNESS_DOCS_PATH`
   env var) is set and the topic's page exists under it (`{path}/{topic}.md`, or
   `{path}/capabilities/overview.md` for `capabilities`), that file is read and returned.
2. **Remote fetch** -- otherwise the page is fetched from the same path under
   `https://raw.githubusercontent.com/pydantic/pydantic-ai/main/docs/`.
