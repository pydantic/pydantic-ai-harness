...
from pydantic_ai_harness._warn import WORKING_DIR_IS_THE_WORKSPACE, HarnessDeprecationWarning, warn_argument_ignored
...
class RepoContext(AbstractCapability[AgentDepsT]):
    ...
    _cached_working_directory: Path | None = field(default=None, init=False, repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        if self.workspace_dir is not None:
            warn_argument_ignored('RepoContext', 'workspace_dir', WORKING_DIR_IS_THE_WORKSPACE)
    ...
    async def for_run(self, ctx: RunContext[AgentDepsT]) -> RepoContext[AgentDepsT]:
        ...
        run._seen_dirs = set()
        run._cached_working_directory = None
        return run
    ...
    async def before_run(self, ctx: RunContext[AgentDepsT]) -> None:
        ...
        workspace_dir = await self._working_directory(workspace)
        home = None
    ...
    def _render_instructions(self) -> str | None:
        ...
        if self._context_files:
            assert self._cached_working_directory is not None, '`before_run` resolves it before loading files'
            parts.append(render_context_files(self._context_files, relative_to=self._cached_working_directory))
    ...
    async def _on_file_traversal(
        ...
        if not self.nested_traversal:
            return
        workspace = await self._working_directory(ctx.workspace)
    ...
    def _label(self, path: Path) -> str:
        assert self._cached_working_directory is not None, 'a note is only rendered once the directory is resolved'
        try:
            return path.relative_to(self._cached_working_directory).as_posix()
        except ValueError:
            return path.as_posix()

    async def _working_directory(self, workspace: Workspace) -> Path:
        if self._cached_working_directory is None:
            self._cached_working_directory = Path(await workspace.working_dir())
        return self._cached_working_directory
