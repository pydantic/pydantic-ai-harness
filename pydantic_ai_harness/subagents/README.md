...
- **The parent's sandbox is forwarded.** Each sub-agent runs in the parent run's sandbox, including a deferred connection or an `UnavailableSandbox`, so a delegated task works in the same environment; a sub-agent's own sandbox capability is not consulted.
